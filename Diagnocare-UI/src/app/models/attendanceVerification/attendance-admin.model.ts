...
/**
 * Mirrors the API's `AttendanceEventType`. The numbers are the wire format — the API
 * serialises enums as numbers, so these values are load-bearing and must not be
 * renumbered on one side alone.
 */
export enum AttendanceEventType {
  ...
  ManualCorrection   = 7,
  AdminOverride      = 8,
  QrValidationFailed = 9,
  DeviceEnrolled     = 10,
  DeviceRevoked      = 11,
}

/** The event types worth offering as a filter — the attempt rows are noise on this screen. */
export const AUDIT_FILTER_TYPES: ReadonlyArray<{ value: AttendanceEventType; label: string }> = [
  { value: AttendanceEventType.CheckInSuccess,     label: 'Check-in accepted' },
  { value: AttendanceEventType.CheckInFailed,      label: 'Check-in refused' },
  { value: AttendanceEventType.CheckOutSuccess,    label: 'Check-out accepted' },
  { value: AttendanceEventType.CheckOutFailed,     label: 'Check-out refused' },
  { value: AttendanceEventType.ManualCorrection,   label: 'Manual correction' },
  { value: AttendanceEventType.QrValidationFailed, label: 'QR rejected' },
  { value: AttendanceEventType.DeviceEnrolled,     label: 'Device enrolled' },
  { value: AttendanceEventType.DeviceRevoked,      label: 'Device revoked' },
];
