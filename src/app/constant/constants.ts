...
export const controllerEndpoints = {
  ...
  /**
   * Device-verified attendance — the employee's phone and the laboratory kiosk.
   *
   * Separate from `attendance` above, which is the administrator's marking grid and the
   * correction-request workflow. They authenticate differently: this one accepts an
   * enrolled device token as well as a Bearer session, because the handset checking in
   * has deliberately never signed in.
   */
  attendanceCheckIn: 'api/AttendanceCheckIn/',
  /** Attendance administration — locations, shifts, assignments, device enrolment. */
  attendanceSetup: 'api/AttendanceSetup/',
  /** Manual corrections and the append-only attendance audit trail. */
  attendanceAudit: 'api/AttendanceAudit/',
};
...
export const apiEndpoints = {
  ...
  // ── Device-verified attendance (api/AttendanceCheckIn/) ───────────────────
  /** The whole employee screen in one call — name, shift, today's times, what is allowed. */
  attendanceToday:          'today',             // GET   api/AttendanceCheckIn/today
  attendanceCheckIn:        'check-in',          // POST  api/AttendanceCheckIn/check-in
  attendanceCheckOut:       'check-out',         // POST  api/AttendanceCheckIn/check-out
  /** The caller's own history. No employee-id parameter by design — identity is the token. */
  attendanceHistory:        'history',           // GET   api/AttendanceCheckIn/history
  /**
   * The kiosk's current QR rotation. Takes no location: the backend reads it from the
   * kiosk token's claim, which is what stops one site's display asking for another's code.
   */
  attendanceKioskQr:        'qr',                // GET   api/AttendanceCheckIn/qr

  // ── Attendance setup (api/AttendanceSetup/) ───────────────────────────────
  attendanceLocations:              'locations',                // GET/POST api/AttendanceSetup/locations
  attendanceAssignEmployeeLocation: 'locations/assign-employee', // POST     api/AttendanceSetup/locations/assign-employee
  /** Appended to `locations/{id}/` — the live rotation, so previewing never invalidates it. */
  attendanceQrPreview:              'qr-preview',               // GET      api/AttendanceSetup/locations/{id}/qr-preview
  attendanceShifts:                 'shifts',                   // GET/POST api/AttendanceSetup/shifts
  attendanceShiftAssignments:       'shifts/assignments',       // GET      api/AttendanceSetup/shifts/assignments
  attendanceAssignShift:            'shifts/assign',            // POST     api/AttendanceSetup/shifts/assign
  attendanceDevices:                'devices',                  // GET/POST api/AttendanceSetup/devices
  /** Appended to `devices/{id}/`. Revocation is immediate — the token is checked per call. */
  attendanceRevokeDevice:           'revoke',                   // POST     api/AttendanceSetup/devices/{id}/revoke

  // ── Attendance audit and correction (api/AttendanceAudit/) ────────────────
  attendanceDaily:          'daily',             // GET   api/AttendanceAudit/daily
  /** Appended to `{attendanceId}/`. Requires a reason — the service refuses without one. */
  attendanceCorrection:     'correction',        // POST  api/AttendanceAudit/{attendanceId}/correction
  attendanceEvents:         'events',            // GET   api/AttendanceAudit/events
};
